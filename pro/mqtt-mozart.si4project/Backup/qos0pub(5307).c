...
int makeJson(char *p,sds_results_file *sds_results_msg)
{
	...
	//保存语音和app的所有操作
	if(strcmp(sds_results_msg->output, "小乐处于工作状态，请稍后再试") && strcmp(sds_results_msg->domain, "APP_WIFI") && strcmp(sds_results_msg->domain, "APP_COMMAND_OK") &&  strcmp(sds_results_msg->input, "BNGO版本") && strcmp(sds_results_msg->input, "ASR_INFO"))
